...
SCRIPT_NAME_DEFAULT = (
    os.path.join("stereo_sync_stack_v14", "stereo_sync_stack_v14.exe")
    if getattr(sys, "frozen", False)
    else "stereo_sync_stack_v14.py"
)
VIDEO_FILETYPES = [
...
def get_python_command():
    # Normal .py launch: keep current behavior and use the same interpreter/venv.
    if not getattr(sys, "frozen", False):
        return [sys.executable]

    # Frozen EXE launch: find a real Python interpreter for running external .py scripts.
    ...
    raise RuntimeError(
        "Could not find a Python interpreter to run the selected processing script.\n\n"
        "When using the packaged GUI EXE, either:\n"
        "1) place python.exe next to the EXE, or\n"
        "2) make sure Python is installed and available on PATH."
    )
...
class StereoSyncGUI(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            ...
            text=(
                ...
                "It can launch either the processing .py script or a compiled processing .exe.\n"
                "When a .py is selected, it uses the current Python interpreter.\n"
                "When an .exe is selected, it runs that executable directly.\n"
    ...
    def _choose_primary_input_path(self):
        left = self.vars["left_video"].get().strip()
        right = self.vars["right_video"].get().strip()
        if left:
            return left
    ...
    def _build_auto_output_path(self):
        input_path = self._choose_primary_input_path()
        if not input_path:
            return None
        ...
        ext = os.path.splitext(current_output)[1] if current_output else ""
        if not ext:
            ext = ".mkv"

        input_stem = os.path.splitext(os.path.basename(input_path))[0].strip()
        if not input_stem:
            input_stem = "output"

        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        return os.path.join(input_folder, f"{input_stem}_{timestamp}{ext}")
    ...
    def _maybe_auto_output_from_inputs(self, force=False):
        ...
        suggested = self._build_auto_output_path()
        if not suggested:
            return
    ...
    def build_command(self, mode=None):
        ...
        if script_ext == ".exe":
            cmd = [script_path, left_video, right_video, "--mode", mode]
        else:
            cmd = get_python_command() + [script_path, left_video, right_video, "--mode", mode]
    ...
    def stop_process(self):
        ...
        try:
            ...
            self._append_console("\nStop requested. Terminating process tree...\n")
            self.status_var.set("Stopping...")
        except Exception as exc:
            self._append_console(f"\nCould not terminate process tree: {exc}\n")
