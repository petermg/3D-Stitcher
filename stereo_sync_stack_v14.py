...
def map_amd_quality_preset(preset):
    p = str(preset).strip().lower()
    if p in {"ultrafast", "superfast", "veryfast", "faster", "fast"}:
        return "speed"
    if p in {"medium", "default", "balanced"}:
        return "balanced"
    return "quality"


def map_svt_preset(preset):
    p = str(preset).strip().lower()
    if p.isdigit():
        val = int(p)
        if 0 <= val <= 13:
...
def map_generic_quality_to_nvenc_cq(crf):
    return str(clamp_int(crf, 0, 51))


def map_generic_quality_to_amd_qvbr(crf):
    generic = clamp_int(crf, 1, 51)
    # Preserve the old effective default behavior:
    # generic CRF 35 -> AMD qvbr_quality_level 18
    return str(clamp_int(53 - generic, 1, 51))


def map_generic_quality_to_svt_crf(crf):
    return str(clamp_int(crf, 0, 63))
...
def build_render_command(
    ...
    if encoder == "nvidia":
        nvenc_cq = map_generic_quality_to_nvenc_cq(crf)
        video_encode_args = [
            "-c:v", "av1_nvenc",
            "-rc", "vbr",
            "-cq", nvenc_cq,
        ...
        amd_quality = map_amd_quality_preset(preset)
        amd_qvbr = map_generic_quality_to_amd_qvbr(crf)
        video_encode_args = [
            "-c:v", "av1_amf",
            "-quality", amd_quality,
            "-rc", "qvbr",
            "-qvbr_quality_level", amd_qvbr,
            "-preencode", "true",
        ...
        svt_preset = map_svt_preset(preset)
        svt_crf = map_generic_quality_to_svt_crf(crf)
        video_encode_args = [
            "-c:v", "libsvtav1",
            "-crf", svt_crf,
            "-preset", svt_preset,
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=35,
        help="Generic quality control. Lower = better quality / larger files. Mapped to NVENC -cq, AMD qvbr_quality_level (inverted), and libsvtav1 -crf."
    )
    parser.add_argument("--preset", default="slow")
    parser.add_argument("--encoder", choices=["nvidia", "amd", "cpu"], default="nvidia", help="Video encoder backend: nvidia=av1_nvenc, amd=av1_amf, cpu=libsvtav1.")
